...
## Clean up (manual object removal)

In Step 1, each loaded photo shows a **Clean up** button. It opens an editor
where you brush over a small unwanted object — a stray cable, a shoe, a bit
of clutter — and the tool fills that area in.

**Read this before you rely on it: this is not AI object removal.** There's
no model detecting objects or reconstructing what "should" be behind them.
It's a local algorithm (multi-resolution diffusion) that spreads nearby
colors inward from the edges of whatever you marked — closer to a smart
smudge than Photoshop's Generative Fill. That means:

- It works reasonably well on **small objects against a plain, low-detail
  surface** — a bare wall, floor, or ceiling.
- It will look smudgy or obviously wrong on **patterned/textured
  backgrounds** (tile, wallpaper, wood grain) or **large objects**, because
  it has no way to reconstruct a repeating pattern — it just blends flat
  color.
- Runs at up to 2000px on the long edge for speed; more than enough for
  every export size this tool offers, but it's not editing at the photo's
  full original resolution.
- Nothing is sent anywhere — it's plain canvas pixel math, same as the rest
  of the tool.
- Always check the result in the editor before saving. If it looks off,
  hit "Reset to original" and just retake or physically fix the shot
  instead — that will always beat any filter.
- A real AI cleanup (Clipdrop, Adobe Firefly, etc.) would give genuinely
  good results but needs an API key, a backend proxy, and costs money per
  image — a different tool than this one intentionally is.
