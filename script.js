...
// Past this many loaded photos, warn the user that browser-tab memory/CPU
// (not any limit this code imposes) becomes the practical ceiling. Purely
// advisory — processing still proceeds regardless of the count.
const LARGE_BATCH_WARN_THRESHOLD = 300;
...
let runCancelled = false; // set true by the Cancel button mid-run
...
const batchWarningEl = $("batch-warning");
const failedSummaryEl = $("failed-summary");
...
function updateSaveModeNote() {
  ...
  if (sourceDirHandle) {
    saveModeNoteEl.textContent =
      "Direct-save is on: processed photos will be written straight into this folder, named after each original photo plus its output size (e.g. sunset_1024x768.jpg) — no download needed.";
...
function refreshPhotoCount() {
  ...
  if (batchWarningEl) {
    if (photoFiles.length > LARGE_BATCH_WARN_THRESHOLD) {
      batchWarningEl.hidden = false;
      batchWarningEl.textContent =
        `That's ${photoFiles.length} photos — large batches are limited by your browser's memory, ` +
        `not by anything this tool imposes. It should still work, but expect it to take a while and ` +
        `consider processing in smaller groups if the tab struggles.`;
    } else {
...
async function processOneFile(file, settings, index, usedNames) {
  ...
  const outName = renameForOutput(file.name, outW, outH, settings.format, usedNames);
  return { name: outName, blob: finalBlob, flags };
}

// Strips the extension and any characters that aren't safe across
// filesystems/zip tools, so "Living Room #3 (final).JPG" becomes
// "Living-Room-3-final".
function sanitizeBaseName(originalName) {
  const withoutExt = (originalName || "photo").replace(/\.[^./\\]+$/, "");
...
// Names each output after its original source photo plus the actual output
// dimensions (e.g. "living-room_1024x768.jpg"), per the README. usedNames
// is a Set scoped to one run, so two source photos that sanitize to the
// same base name (or that share an original name across sub-folders)
// still get distinct, non-clobbering output filenames within that run.
function renameForOutput(originalName, width, height, format, usedNames) {
  const ext = format === "png" ? "png" : "jpg";
  const base = sanitizeBaseName(originalName);
  const stem = `${base}_${width}x${height}`;
...
btnRun.addEventListener("click", async () => {
  ...
  if (failedSummaryEl) failedSummaryEl.hidden = true;

  btnRun.disabled = true;
  progressWrap.hidden = false;
  runCancelled = false;
  if (btnCancelRun) {
    btnCancelRun.hidden = false;
  ...
  for (let i = 0; i < photoFiles.length; i++) {
    ...
    try {
      const { name, blob, flags } = await processOneFile(photoFiles[i], settings, i, usedNames);
      const url = URL.createObjectURL(blob);
      ...
      failedFiles.push(photoFiles[i].name);
    }
  ...
  if (btnCancelRun) btnCancelRun.hidden = true;
  ...
  if (results.length && sourceDirHandle) {
    try {
      ...
      saveStatusEl.textContent = `Saved ${results.length} photo${results.length === 1 ? "" : "s"} straight into the picked folder, named after each original photo plus its output size (any file that already had that exact name was overwritten).`;
    } catch (err) {
  ...
  if (failedSummaryEl) {
    if (failedFiles.length) {
      failedSummaryEl.hidden = false;
      const shown = failedFiles.slice(0, 8).join(", ");
      const extra = failedFiles.length > 8 ? ` and ${failedFiles.length - 8} more` : "";
      failedSummaryEl.textContent =
        `${failedFiles.length} photo${failedFiles.length === 1 ? "" : "s"} couldn't be processed and ` +
        `${failedFiles.length === 1 ? "was" : "were"} skipped: ${shown}${extra}.`;
    } else if (wasCancelled) {
      failedSummaryEl.hidden = false;
      failedSummaryEl.textContent = `Cancelled — processed ${processedCount} of ${photoFiles.length} photos before stopping.`;
    }
  }
});
...
if (btnCancelRun) {
  btnCancelRun.addEventListener("click", () => {
    ...
    btnCancelRun.textContent = "Cancelling…";
  });
}
...
function appendFrame(name, url, flags) {
  ...
  div.innerHTML = `<img src="${url}" alt="${name}" />${badges}<a class="frame__dl" href="${url}" download="${name}" title="Download ${name}" aria-label="Download ${name}">&#8681;</a><span class="frame__tag">${name}</span>`;
