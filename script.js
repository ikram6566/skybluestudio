...
async function processOneFile(file, settings) {
  ...
  const outName = renameForOutput(file.name, settings.format, outW, outH);
  return { name: outName, blob: finalBlob, flags };
}

function renameForOutput(originalName, format, w, h) {
...
btnRun.addEventListener("click", async () => {
  ...
  for (let i = 0; i < photoFiles.length; i++) {
    ...
    try {
      const { name, blob, flags } = await processOneFile(photoFiles[i], settings);
      const url = URL.createObjectURL(blob);
